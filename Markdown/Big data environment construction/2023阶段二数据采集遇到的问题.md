...
- ## 7、[通信链路故障](#CommunicationsLinkFailure)

- ## 8、[hive的配置文件没有拷贝到spark/config下](#hiveFile)
...
## 7、<a id="CommunicationsLinkFailure">通信链路故障</a>
...
- ### 原因
  ...
  - 1、MySQL驱动包没有拷贝到spark/jars下面
  - 2、hive的配置文件hive-site.xml 没有拷贝到spark/conf下面
  - 3、连接mysql的url的主机名写错
...
- ### 解决
  ...
  - 三台机器都要拷贝
    ...
    - 1、scp /opt/soft/mysql-connector-java-5.1.37.jar bigdata2:/usr/local/src/spark/jars/
    - 2、cp /usr/local/src/hive/conf/hive-site.xml /usr/local/src/spark/conf
...
## 8、<a id="hiveFile">hive的配置文件没有拷贝到spark/config下</a>

- 问题

  ```
  2023-09-12 10:33:45,397 WARN metastore.ObjectStore: Failed to get database ods, returning NoSuchObjectException
  org.apache.spark.sql.AnalysisException: Table or view not found: ods.brand_info; line 1 pos 71;
  'Project [unresolvedalias('string('if(isnull('max('modified_time)), , 'modified_time)), None)]
  +- 'UnresolvedRelation [ods, brand_info], [], false
  ```

- 原因

  - hive的配置文件没有拷贝到spark/config下，导致spark无法识别到hive
  - mysql的驱动没有拷贝到spark/jars下

- 解决

  ```
  scp -r /opt/module/apache-hive-3.1.2-bin/conf/hive-site.xml bigdata1:/opt/module/spark-3.1.1-bin-hadoop3.2/conf/
  scp -r /opt/module/apache-hive-3.1.2-bin/conf/hive-site.xml bigdata2:/opt/module/spark-3.1.1-bin-hadoop3.2/conf/
  scp -r /opt/module/apache-hive-3.1.2-bin/conf/hive-site.xml bigdata3:/opt/module/spark-3.1.1-bin-hadoop3.2/conf/
  
  scp /opt/software/mysql-connector-java-5.1.37.jar bigdata1:/opt/module/spark-3.1.1-bin-hadoop3.2/jars/
  scp /opt/software/mysql-connector-java-5.1.37.jar bigdata2:/opt/module/spark-3.1.1-bin-hadoop3.2/jars/
  scp /opt/software/mysql-connector-java-5.1.37.jar bigdata3:/opt/module/spark-3.1.1-bin-hadoop3.2/jars/
  ```
