...
|----------------|--------------------------------------------------------------------------------------------------------------|---------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------|-------------------------------|
| 读文件            | <ul><li>pandas.read_csv('file.csv', sep='\t')</li><li>pandas.read_excel('file.xlsx', skiprows=2)</li></ul>   | 从文件中读取数据并返回一个DataFrame对象                    | <ul><li>sep='\t'</li><li>skiprows=2</li></ul>                                                                                                                                                                                                | <ul><li>默认情况 read_csv 函数读取逗号分隔的文件，sep='\t' 显示指明使用制表符分隔 </li><li>读excel文件前两行是空的时，跳过前两行空行</li></ul>   | 文件以什么来分隔                      |
| 导出             | <ul><li>df.to_csv()</li><li>df.to_excel()</li></ul>                                                          | DataFrame数据导出为相应文件（.csv / .xlsx）            | <ul><li>sep=","</li><li>index=False</li><li>header=True</li></ul>                                                                                                                                                 | <ul><li>指定数据分隔符</li><li>避免写入行索引</li><li>将文件的第一行作为列名</li></ul>                                       | 路径后面要跟上文件名                    |
