# 1、根据dwd层表统计每个地区、每个国家、每个月下单的数量和下单的总金额，
...
select n.NATIONKEY, n.NAME, r.REGIONKEY, r.NAME, sum(o.TOTALPRICE), count(*), year(o.ORDERDATE), month(o.ORDERDATE)
from region as r
...
-- 存入MySQL数据库shtd_store的nationeverymonth表（表结构如下）中,然后根据订单总数、消费总额、国家表主键三列均逆序排序的方式，查询出前5条
select *
from (select n.NATIONKEY as nk, n.NAME as nn, r.REGIONKEY as rk, r.NAME as rn,
             sum(o.TOTALPRICE) as s1, count(*) as c1, year(o.ORDERDATE) as y1, month(o.ORDERDATE) as m1
      from region as r
      inner join nation as n on r.REGIONKEY=n.REGIONKEY
      inner join customer as c on n.NATIONKEY=c.NATIONKEY
      inner join orders as o on c.CUSTKEY=o.CUSTKEY
      group by r.REGIONKEY, r.NAME, n.NATIONKEY, n.NAME, year(o.ORDERDATE), month(o.ORDERDATE)) as t1
...
-- 某年所有国家平均消费额
select year(o.ORDERDATE),  avg(o.TOTALPRICE) as avg2
from customer as c
inner join orders as o on c.CUSTKEY=o.CUSTKEY
inner join nation as n on c.NATIONKEY=n.NATIONKEY
...
# 3、根据dwd层表统计连续两个月下单并且下单金额保持增长的用户，订单发生时间限制为大于等于某年，
...
-- 每月下用户的情况
select year(o.ORDERDATE), c.CUSTKEY, c.NAME,
       sum(o.TOTALPRICE), count(*)
from customer as c
inner join orders as o on c.CUSTKEY=o.CUSTKEY
group by year(o.ORDERDATE), month(o.ORDERDATE), c.CUSTKEY, c.NAME
