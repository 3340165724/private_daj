...
object OdsHBaseToDwd923 {
  def main(args: Array[String]): Unit = {
    ...
    for (i <- 0 until ods_tables.length) {
      ...
      val ods_df = spark.sql(s"select * from ods.${ods_table} where etl_date='20230922'").drop("etl_date")
  ...
  def readHBaseDf(hbaseNmae: String, odsDf: DataFrame, spark: SparkSession): DataFrame = {
    ...
    val table = connection.getTable(TableName.valueOf(hbaseNmae))

    val scan = new Scan()
    scan.setFilter(new RowFilter(CompareOperator.EQUAL, new SubstringComparator("20230922")))
    ...
    val list = resultScanner.iterator().asScala.map(x => {
      val values = fields.map(field => {
        ...
        val value = tp match {
          case "login" => Bytes.toLong(x.getValue("info".getBytes(), name.getBytes()))
